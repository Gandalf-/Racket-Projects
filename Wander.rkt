...
;Choose number of segments in body
...
(define segment-slider
  (new slider%
       ...
       (callback (lambda (b e) 
                   ...
                   (when (and (> num-segments 20) (= (send speed-slider get-value) 1))
      ...
      (begin
        (let loop ((i 0))
          (unless (= i num-segments)
            (let ((head (make-object point% (random frame-width) (random frame-height))))
  ...
  (class object%
    ...
    (define (move)
      ...
      (define (find-new-x angle)
        (* (cos angle) move-distance))
      
      (define (find-new-y angle)
        (* (sin angle) move-distance))
      ...
      ;Slave function that recursively goes through the body list
      (define (move-slave input)
        (unless (empty? input)
          (if (equal? (car input) (car body))
              ;Head case, choose new postion and move head there
              (begin                 
                (let ((angle (+ previous-angle (* (random-sign) (+ (/ (send spread-slider get-value) 10) (random 1))))))
                ;(let ((angle (- previous-angle (* (random-sign) (random (send spread-slider get-value))))))
                  
                  ;Assign nex x coord
                  (send (car input) set-x (modulo (round (+ (send (car input) get-x)
                                                            (find-new-x angle)))
                                                  (send canvas get-width)))
                  ;Assign new y coord
                  (send (car input) set-y (modulo (round (+ (send (car input) get-y)
                                                            (find-new-y angle)))
                                                  (send canvas get-height)))
                  (set! previous-angle angle)))
              ;Body case, set everything to next in reversed list
              (begin
                (send (car input) set-x (send (car (cdr input)) get-x))
                (send (car input) set-y (send (car (cdr input)) get-y))
                (move-slave (cdr input))))))
      
      (move-slave (reverse body))
      )
    (super-new)))
  ...
  (thread
   (lambda ()
     (let loop ((i 0))
       ...
       (send canvas refresh)
       (sleep (/ (send speed-slider get-value) 50))
       (loop (+ 1 i))))))
